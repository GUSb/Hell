...
public class CollectionsPerformance {
  ...
  public List<Integer> generateArrayList(int size, int randomization) {
    ...
    long time = CollectionsHelper
        .countTimeMillis(() -> CollectionsHelper.generate(size, randomization, tokens));
    System.out
        .println(String.format("Generated %s nodes of ArrayList in time - %s ms", size, time));
    return tokens;
  }
  ...
  public List<Integer> generateLinkedList(int size, int randomization) {
    ...
    long time = CollectionsHelper
        .countTimeMillis(() -> CollectionsHelper.generate(size, randomization, tokens));
    System.out
        .println(String.format("Generated %s nodes of LinkedList in time - %s ms", size, time));
    return tokens;
  }
  ...
  public void sort(List<Integer> input) {
    ...
    System.out
        .println(String.format("Sort of %s in time - %s ms", input.getClass().getName(), time));
  }
